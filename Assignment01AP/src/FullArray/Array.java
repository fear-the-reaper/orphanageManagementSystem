...
public class Array{
	...
	/*	array.getValues(); //get values from the user.
		...
		array.insert(10,0);//insert num=10 at pos=4
		array.display();   //display the array.

		array.delete(3);   //delete num at pos=3 from the array.
		array.display();   //display the array.

		System.out.println(array.linearSearch(35)); //check 40 is present in the array using linearSearch.

		array.bubbleSort();//sort the array.
		array.display();   //display the array.

		System.out.println(array.binarySearch(4,0,size));//search for num=56 using Binary Search.
	*/
		array.insert(8,0);
		array.insert(5,1);
		array.insert(3,2);
